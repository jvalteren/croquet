...
(function (modules) {
				...
				var prototype = node.derive({
				    ...
				    getCaretCoords: function(ordinal) {
				        var node = this.byOrdinal(ordinal), b;
                                        var caretWidth = Math.min(3, Math.max(.5, Math.floor(this.resolution/2)));
				        if (node) {
  ...
  exports.create = function createEditor(resolution, width, height, clearFunction) {
    return carotaDoc().initialize(resolution, width, height, clearFunction);
  }
  ...
  var CarotaPrototype = Object.getPrototypeOf(carotaDoc()) || carotaDoc().__proto__;

  CarotaPrototype.initialize = function(width, height, resolution, clearFunction) {
  ...
  CarotaPrototype.changeLine = function(ordinal, direction) {
  ...
  CarotaPrototype.endOfline = function(ordinal, direction) {
    var originalCaret = this.getCaretCoords(ordinal), newCaret;
    while (!exhausted(this, ordinal, direction)) {
      ordinal += direction;
      newCaret = this.getCaretCoords(ordinal);
  ...
  CarotaPrototype.resize = function(width, height, resolution) {
    this.screenWidth = width;
    this.screenHeight = height;
    this.resolution = resolution;
    let {canvas} = this;
    canvas.width = closestPow2(resolution * width);
    canvas.height = closestPow2(resolution * height);
  ...
  CarotaPrototype.paint = function() {
    ...
    let {
        ...
        absScrollTop = scrollTop*this.scaleY,
        absWidth = screenWidth*scaleX,
        absHeight = screenHeight*scaleY;
 
    var ctx = canvas.getContext('2d');
    ctx.save();
    if (clearFunction) clearFunction(ctx, 0, 0, canvas.width, canvas.height);
    else ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.translate(0, -absScrollTop);
    this.draw(ctx, rect(absScrollLeft, absScrollTop, canvas.width, canvas.height));
  ...
  CarotaPrototype.drawScrollbar = function(ctx) {
    let {screenHeight, scaleY, scaleX, scrollTop} = this;
    let {l, t, h, w, r, b} = this.scrollbarBounds();
    ctx.save();
    ctx.fillStyle = "rgba(100, 100, 100, 0.5)";
    ctx.fillRect(l, scaleY*scrollTop, w+3*scaleX, scaleY*screenHeight);

    ctx.lineWidth = 2;
    ctx.strokeStyle = "gray";
    // ctx.beginPath();
    // ctx.moveTo(l, scaleY*scrollTop);
    // ctx.lineTo(l, scaleY*scrollTop+scaleY*screenHeight);
    // ctx.stroke();
    var gradient = ctx.createLinearGradient(l-5, 0, r+12, 0);
    gradient.addColorStop(0, 'white');
    gradient.addColorStop(1, 'black');
    ctx.fillStyle = gradient;
    roundRect(ctx, l+3, t, w, h, 5, true, true);
    ctx.restore()
  }

  CarotaPrototype.scrollbarBounds = function() {
    var {
          scaleX,
          scaleY,
          scrollTop,
          screenHeight,
          screenWidth,
          relativeScrollBarWidth,
          frame
        } = this,
        fullHeight = frame.bounds().h / scaleY,
        scrollBarRelativeHeight = screenHeight / fullHeight,
        scrollBarWidth = relativeScrollBarWidth * screenWidth * scaleX,
        scrollBarLeft = screenWidth*scaleX - scrollBarWidth - 3,
        scrollBarTop = scrollTop * scrollBarRelativeHeight * scaleY + scrollTop * scaleY - 3,
        minHeight = screenHeight/100 * 5 * scaleY,
        scrollBarHeight = Math.max(minHeight, screenHeight * scrollBarRelativeHeight * scaleY - 3);

    return rect(scrollBarLeft, scrollBarTop, scrollBarWidth, scrollBarHeight);
  }

  CarotaPrototype.scrollBy = function(deltaX, deltay) {
    this.setScroll(this.scrollLeft = deltaX, this.scrollTop + deltay);
  }

  CarotaPrototype.setScroll = function(scrollLeft, scrollTop) {
    let {screenHeight, scaleY} = this,
        docHeight = this.frame.bounds().h
    // this.scrollLeft = ...
    this.scrollTop = Math.max(0, Math.min(docHeight/scaleY - screenHeight, scrollTop));
    this.contentChanged.fire();
  }

  CarotaPrototype.scrollRangeIntoView = function({start, end}) {
    let {scaleY} = this,
        t = this.getCaretCoords(start).t / scaleY,
        b = this.getCaretCoords(end).b / scaleY;
    let vBounds = this.visibleBounds(),
        deltaY = 0
    if (b > vBounds.b) deltaY = b - vBounds.b;
    if (t < vBounds.t) deltaY = t - vBounds.t;
    this.scrollBy(0, deltaY);
  }

  CarotaPrototype.scrollCursorIntoView = function() {
    return this.scrollRangeIntoView(this.selection);
  }

  CarotaPrototype.visibleBounds = function() {
    return rect(this.scrollLeft, this.scrollTop, this.screenWidth, this.screenHeight);
  }

  CarotaPrototype.updateTextArea = function() {
    this.focusChar = this.focusChar === null ? this.selection.end : this.focusChar;
    var endChar = this.byOrdinal(this.focusChar);
    this.focusChar = null;
  }

  CarotaPrototype.mouseDown = function(x,y) {
    let {screenWidth, scaleX, relativeScrollBarWidth} = this,
        scrollBarWidth = relativeScrollBarWidth * screenWidth * scaleX,
        scrollBarLeft = screenWidth*scaleX - scrollBarWidth - 3;

    if (x >= scrollBarLeft) {
      this.scrollBarClick = "clicked";
    } else {
  ...
  CarotaPrototype.mouseMove = function(x,y){
    ...
    if (this.scrollBarClick) {
      let {screenWidth, screenHeight, scaleY, scrollTop} = this,
          absDocHeight = this.frame.bounds().h,
          absScrollTop = scrollTop*scaleY,
          absScrollHeight = screenHeight*scaleY,
          docHeight = absDocHeight / scaleY,
          relScrollTop = (y - absScrollTop) / absScrollHeight,
          newScrollTop = Math.min(docHeight * relScrollTop, docHeight - screenHeight);
      this.scrollBarClick = "move";
      this.setScroll(0, newScrollTop);
      this.paint();
    }
  }

  CarotaPrototype.mouseUp = function(x,y){
    if (this.scrollBarClick) {
      if (this.scrollBarClick === "clicked") {
        let {t, b} = this.scrollbarBounds();
        if (y < t) this.scrollBy(0, -this.screenHeight)
        else if (y > b) this.scrollBy(0, this.screenHeight)
        this.paint();
      }
