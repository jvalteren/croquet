...
async function initIconSVGData() {  
  let iconDocString = await resource(
      ...
      iconDoc = new DOMParser().parseFromString(iconDocString, "application/xml");
  glyphs = Array.from(iconDoc.querySelectorAll("glyph"));
}

async function glyphFor(iconName) {
  if (!Icons[iconName])
    throw new Error("No such icon: " + iconName);
  if (!glyphs) await initIconSVGData();
  return glyphs.find(n => n.getAttribute("unicode") === Icons[iconName].code);
}

async function pathFor(iconName) {
  return (await this.glyphFor(iconName)).getAttribute("d");
}

export async function shapeFor(iconName) {
  let shapes = transformSVGPath(await pathFor(iconName)).toShapes(true);
  if (!shapes.length)
    throw new Error("Could not extract path for icon " + iconName)
  if (shapes.length > 1)
    console.warn("More than one path for icon" + iconName + "???");
  return shapes[0];
}

export async function meshFor(iconName, extrudeSettings, materialSettings) {
  extrudeSettings = {
    steps: 2,
    amount: 1,
    bevelEnabled: true,
  ...
  materialSettings = {
    color: Globals.standardColor.getHex(),
    emissive: 0x222222, opacity:0.9,
    transparent:true, side: THREE.FrontSide,
    ...materialSettings
  };

  var material = new THREE.MeshStandardMaterial(materialSettings),
      geometry = new THREE.ExtrudeGeometry(await shapeFor(iconName), extrudeSettings);
  geometry.normalize();
  return new THREE.Mesh(geometry, material);
}

export async function objFor(iconName, extrudeSettings, materialSettings) {
  let mesh = await meshFor(iconName, extrudeSettings, materialSettings);
  return new TObject(null, null, mesh);
}
